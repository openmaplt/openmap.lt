...
### AI chat search (Mistral) — admin-only, why a Route Handler, and the SQL-injection decision
...
The model/provider is swappable without touching the route or `aiSearchClient.ts`'s call sites: `src/config/aiModel.ts` holds one `"provider:model-id"` string (`AI_MODEL_ID`), resolved through an `ai` SDK `createProviderRegistry` in `src/lib/aiSearchClient.ts`. To switch providers: install that provider's `@ai-sdk/*` package, add it to the registry, change the one string. All providers in the registry share a single generic `AI_MODEL_API_KEY` env var (not a per-provider key) — safe because only the provider actually selected by `AI_MODEL_ID` is ever called; an unregistered/invalid key on the unused provider is harmless since `registry.languageModel()` never touches it.

Two version-specific gotchas hit during implementation, worth knowing before touching this code again: (1) `@ai-sdk/*` providers' bare defaults (`google`/`createGoogleGenerativeAI()`, `createMistral()`, etc.) each read their own provider-specific env var (`GOOGLE_GENERATIVE_AI_API_KEY`, `MISTRAL_API_KEY`, ...) — pass `apiKey: process.env.AI_MODEL_API_KEY` explicitly to every provider in the registry, or it silently breaks in production. (2) `generateObject` (used for the first LLM call) is marked `@deprecated` in `ai@7` in favor of `generateText({ output: Output.object({ schema }) })` — still fully functional in the installed version, kept for simplicity; re-check if bumping the `ai` package majors. `messages` from `useChat` are `UIMessage[]` and must go through `convertToModelMessages()` before reaching `streamText`/`generateObject`.

**AI-generated routes:** `AiSearchPlanSchema`'s `route` field (`{requested, profile}`) lets the same first LLM call also detect "build a route through these POIs" intent and guess a `foot`/`bike`/`car` profile from the query wording — never reaches SQL, TS-only. When set, `buildAiSearchRoute` (`aiSearchClient.ts`) turns the DB match set's own geometry (already sorted nearest-to-`pos` by `places.ai_search` — no extra `getPoiInfo` roundtrip) into an ordered stop list via `orderNearestNeighbor` (`src/lib/geo.ts` — a greedy nearest-neighbor heuristic, not a real TSP solve; GraphHopper's multi-point `/route` has no optimize mode, it visits points in the exact order given), capped at 8 stops. `pos` is only the ordering's starting reference, never a route point itself — `AiSearchRoutePayload.stops[0]` (the matched POI nearest to `pos`) IS the route start and `stops[last]` IS the end; there's no synthetic "current location" point, since `pos` is just the map center at query time, not a known real position. `route.ts` also calls `fetchGraphHopperRoute` (`src/lib/geo.ts`, shared with the client hook — see gotcha below) server-side to get real distance/time *before* the second LLM call, so `buildRouteReplySystemPrompt` can hand the model the stops pre-ordered (numbered 1..N, describe in that exact order, nothing added/dropped/reordered) plus the real totals — a dedicated prompt variant, not the general search one. Streamed to the client as a transient `data-aiSearchRoute` part. The client (`AiSearchChat.tsx`) surfaces a "Rodyti maršrutą" button whose payload flows to `SearchFeature.tsx`'s `handleShowRoute` (see gotcha below for why it lives there, not in `SearchBox.tsx`), which populates `RouteProvider`'s `routeStart`/`waypoints`/`routeEnd` — the same state `RouteLayer`/`RouteStepsList`/`RouteDetails` already render for manual 2-point routing, plus a few small additions for the multi-stop case: `RouteMarkers` draws numbered amber markers for `waypoints` (green start / red end stay as before), `RoutingInputs` takes a `hideEndpoints` prop (the "Iš kur"/"Į kur" fields can't represent more than 2 points, so `RouteDetails` hides them whenever `waypoints.length > 0`), and `RouteStepsList` swaps each `RouteSign.ViaPoint` instruction's generic GraphHopper text ("Sustojimas N") for the matching waypoint's real name. `waypoints` is deliberately excluded from `urlHash`/`useMapSync` — no share-route support yet.

**Gotcha — `AiSearchChat` must not live inside `SearchBox`:** `SearchBox.tsx` has `if (routingMode) return null` (frees up the screen space RoutingInputs/RouteDetails otherwise share with the search bar). `AiSearchChat` used to render as `SearchBox`'s child, so the moment "Rodyti maršrutą" set `routingMode` true, `SearchBox` — and `AiSearchChat` along with it — unmounted, silently wiping the whole conversation (`useChat`'s state has nowhere else to live). Symptom looked exactly like a Radix Dialog focus-stealing race (a `setTimeout` dodge, the fix for a *different*, real race documented in `MapMenu.tsx`, changed nothing here) but had nothing to do with Radix — always suspect an unmount over a dismissal race when a Sheet's content vanishes AND its internal state resets. Fixed by lifting `AiSearchChat` (plus its `aiChatOpen`/`showAiChat` state and `handleShowRoute`) up to `SearchFeature.tsx`, rendered as a sibling of `SearchBox` — `SearchBox` now only owns the trigger button, calling an `onOpenAiChat` prop.

**Gotcha:** put pure routing math (like `orderNearestNeighbor`/`fetchGraphHopperRoute`) in `src/lib/geo.ts`, never `src/lib/routeUtils.ts`, when it needs to be reachable from server code (e.g. `aiSearchClient.ts`, imported by the `/api/ai-search` Route Handler). `routeUtils.ts` imports `RouteInstruction`/`RouteSign` from `src/hooks/use-routing.ts` for typing — harmless in a client bundle, but that hook file also imports `swr`/`useDebouncedValue`, and Next.js's RSC bundler traces the whole module graph reachable from a server entrypoint: importing anything from `routeUtils.ts` into server code makes the build fail with `useState`/`useSWR`-in-Server-Component errors, even though the specific export you wanted never touches React. `geo.ts` has no such transitive client dependency.
...
### POI descriptions — why not a `places.poi` column

`src/components/descriptions/PoiDescription.tsx` (`places` profile only) lets users holding the `places.description.edit` permission write a shared markdown description per POI, shown to every visitor (logged in or not) right after the attribute list and before the photo gallery/comments/rating (`PoiDetails.tsx`). Stored in `openmap.poi_descriptions` (`object_ref` primary key — one shared row per object, not per-user, unlike `poi_ratings`/`poi_collection_status`), never as a `places.poi` column: `places.poi` is an OSM-sync mirror (`osm_id`, `attr` jsonb, generated columns) owned by an external pipeline (`places.accept_change`/`process_poi_change`/`transfer_id`, role `osm`) that deletes-and-reinserts a row (not just `UPDATE`s it) whenever an OSM object's id/type changes — any column added there would silently lose its value on that path, since the `'N'` (insert) branch of `accept_change` only sets 6 fixed columns. `places.poi.rating` (added by the ratings feature) is the one exception, and only because it's a denormalized read cache whose source of truth lives in `openmap.poi_ratings` and can be recomputed if wiped; descriptions have no equivalent recompute path, so they must live in `openmap` from the start. Fetched live via a server action (`getPoiDescriptionAction`, `src/actions/poiDescriptions.ts`) rather than baked into vector tiles, so an edit is visible to everyone immediately rather than waiting for the next tile refresh; the same action also returns `canEdit` (checked server-side) since the client session (`PublicUser`) only ever exposes `isAdmin`, never per-permission flags.
...
See `.env.example`. Key ones: `DATABASE_URL` (real DB via tunnel during dev), `OSM_CLIENT_ID`/`OSM_CLIENT_SECRET`, `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`, `STVK_API_URL`, `AI_MODEL_API_KEY` (AI chat search, see below).
